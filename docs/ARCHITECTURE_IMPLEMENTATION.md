...
- **Data layer**: Prisma schema exists at `prisma/schema.prisma` (PostgreSQL) with models for users, organizations, memberships, invoices, wallets, ledger entries, payouts, cards, support tickets, jobs, etc. Runtime data is served from a shared data service (`src/lib/data-service.ts`) that mirrors the schema and powers server actions for invites, invoices, payouts, card issuance, onboarding steps, and ledger mutations.
- **API routes**: App Router handlers under `src/app/api/*` wrap the shared data service. Examples: `/api/cron/process` processes queued jobs, `/api/wallet/balances` exposes wallet balances, `/api/wallet/transactions` lists ledger entries, `/api/card/freeze` toggles card status, `/api/health` returns a heartbeat, and `/api/checklist` surfaces onboarding gates.
- **Background jobs**: Jobs are stored via the data service (`Job` records) and processed synchronously by `processJobsAction` when `/api/cron/process` is called. Payout refresh jobs update payout status, create ledger postings, push audit entries, and emit notifications.
- **Notifications & audit**: `src/lib/notification-service.ts` currently logs/collects notification events; `pushAudit` inside the data service writes audit items.
- **Integrations & toggles**: `src/lib/wise-provider.ts` simulates Wise FX quotes/transfers (keyed off `WISE_API_KEY`). No Marqeta adapter yet; card flows rely on wallet balance checks in `issueCardAction`.
...
- **Prisma schema + migrations**: Schema in `prisma/schema.prisma`; no migrations checked in yet. The current data service mirrors the schema inside `src/lib/data-service.ts`.
- **API route conventions**: App Router handlers under `src/app/api/*` use NextResponse, call `auth()` for gating, and delegate to shared server actions/data service (e.g., `/api/wallet/*`, `/api/card/*`, `/api/cron/process`).
- **Job/cron system & webhooks**: Job records live in `src/lib/data-service.ts`; `/api/cron/process` runs `processJobsAction`. Webhook patterns not present yet; provider simulators live in `lib/wise-provider.ts`.
...
5) **Payments & payouts**: Implement invoice funding/release flows that credit wallets, queue payout settlement jobs, and honor Wise environment toggles with FX previews.  
6) **Cards**: Add Marqeta adapter plus card↔wallet coupling: auth creates holds, settlement posts debits, reversals release/credit.  
...
- **Wallet & ledger**: `src/lib/data-service.ts` (current), future `src/modules/wallet/*` for ledger/holds service; UI under `src/app/app/wallet` and `src/app/dashboard/payouts`.
...
- **Invoices & payouts**: `src/lib/data-service.ts` actions, `src/app/app/invoices/*`, `src/app/api/wallet/*` for balances/transactions.
- **Cards**: `src/lib/data-service.ts`, `src/lib/wise-provider.ts` (FX), UI at `src/app/app/cards`, API at `src/app/api/card/*`.
- **Notifications & audit**: `src/lib/notification-service.ts`, audit helpers in `src/lib/data-service.ts`, UI at `src/app/app/audit-logs`.
- **Jobs & webhooks**: `src/lib/data-service.ts` + `/api/cron/process`; future real job runner under `src/modules/jobs/*` and webhooks under `src/app/api/webhooks/*`.
...
- **Data/services**: `src/lib/data-service.ts` (current store + server actions), `src/lib/onboarding-state.ts` (onboarding gates), `src/lib/wise-provider.ts` (Wise simulator), `src/lib/notification-service.ts` (in-app notification collector), `src/lib/idempotency.ts` (helpers).  
